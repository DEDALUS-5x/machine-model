...
#endif
    char* pbase = strrchr(exePath, *separator);
    *pbase = '\0';
    return &exePath[0];
}
#endif
...
{
    return ssGetExecutionInfo((SimStruct*)simstruct)->simulationOptions_.buildDir_;
}
...
const char* fmu_resourceLocation(void* simstruct) {
    ...
    if (saveRapidSimulationMode(-1) > 0) {
        const char* raccelPath = raccelDeployLocation(simstruct);
...
char* fmu_nestedResourceLocation(void* simstruct, const char* uid, const char* model, int root, int fmuVersion) {
    static char nestedResourcesPath[1024];
    memset(&nestedResourcesPath[0], 0, sizeof(nestedResourcesPath));
    const char* resourceLoc = fmu_resourceLocation(simstruct);
#endif
#if FMU_CG_TARGET == FMUCG_NESTEDFMU || FMU_CG_TARGET == FMUCG_PROTECTED_MODEL
    if(fmuVersion <= 2) {
#ifdef _WIN32
...
char* fmu_raccelDeploymentDLLLocation(void* simstruct, const char* deploy, const char* uid, const char* model, const char* dllName, int x64Format)
{
    static char dllPath[1024];
    memset(&dllPath[0], 0, sizeof(dllPath));
    const char* unziplocation = raccelDeployLocation(simstruct);
    strcat(dllPath, unziplocation);
    if (deploy != NULL) {
    	strcat(dllPath, separator);
...
{
    static char grtResourcesPath[1024];
    memset(&grtResourcesPath[0], 0, sizeof(grtResourcesPath));
    if(fmuVersion <= 2) {
        strcat(grtResourcesPath, "file:///");
    }
    strcat(grtResourcesPath, buildDir);
    if (root == 0) {
        strcat(grtResourcesPath, separator);
        strcat(grtResourcesPath, "resources");
    }
    return grtResourcesPath;
}
...
{
    static struct loggerEnvironmentComponent comp = {0};
...
bool canAccessVar(const bool accessClkVars, const unsigned int* activeClocksList, const size_t numActiveClocks, const unsigned int* clkDependsList) {
    bool isNotClockedVar = (clkDependsList[0] == 0);
    bool isAnyOfClkDependenciesActive = false;
    size_t numClkDepends = clkDependsList[0];

    size_t i, j;
    for (i = 1; i <= numClkDepends; ++i) {
        size_t clk = clkDependsList[i];
        for (j = 0; j < numActiveClocks; ++j) {
            if (activeClocksList[j] == clk) {
                isAnyOfClkDependenciesActive = true;
                break;
            }
        }
        if (isAnyOfClkDependenciesActive) {
            break;
        }
    }
    return (accessClkVars ? isAnyOfClkDependenciesActive : isNotClockedVar);
}

bool isVarActive(unsigned int* activeVarList, const size_t activeNvrs, const size_t valueReference) {
    bool isActive = false;
    if (activeNvrs == 0) return isActive;
    int i;
    for (i = 0; i < activeNvrs; i++) {
        if (valueReference == activeVarList[i]) {
            isActive = true;
            break;
        }
    }
