...
PMF_DEPLOY_STATIC NeuDiagnosticTree* neu_diagnostic_manager_get_initial_tree(
    NeuDiagnosticManager const* mgr) {
    /* Const cast required to pass through RTWCG-generated pointer type */
    union {
        NeuDiagnosticTree const* constDiagTree;
...
PMF_DEPLOY_STATIC char* rtw_diagnostics_msg(NeuDiagnosticTree const* tree) {
    ...
    char const*               msg     = printer->mPrint(printer, tree);
...
*/
PMF_DEPLOY_STATIC boolean_T error_buffer_is_empty(void const* ptr) {
    return (ptr == NULL);
}
