...
struct McLinearAlgebraDataTag {
    ...
    PmSparsityPattern const* mSparsityPatternPtr;
};
...
PMF_DEPLOY_STATIC void populate_full_matrix_from_sparsitypattern(
    real_T*                  A,
    PmSparsityPattern const* jacobian_pattern_ptr,
    real_T const*            Ax,
...
PMF_DEPLOY_STATIC McLinearAlgebraData* rtw_linalg_create_data(
    ...
    ne_la_data->mLinvB = (real_T*)pm_allocator_alloc(
        ne_la_data->mAllocatorPtr, sizeof(real_T), jacobian_pattern_ptr->mNumRow);
...
PMF_DEPLOY_STATIC McLinearAlgebraStatus rtw_linalg_numeric(McLinearAlgebra* ne_la,
    ...
    populate_full_matrix_from_sparsitypattern(
        ne_la_data->mLU, ne_la_data->mSparsityPatternPtr, Ax, ne_la_data->mAllocatorPtr);
...
PMF_DEPLOY_STATIC
size_t rtw_linalg_memusage(McLinearAlgebra const* ne_la) {
    McLinearAlgebraData const* ne_la_data = ne_la->mPrivateData;
    size_t const               numrow     = (size_t)ne_la_data->mNumRow;
    size_t const               numcol     = (size_t)ne_la_data->mNumCol;
...
PMF_DEPLOY_STATIC void rtw_linalg_restart(McLinearAlgebra* ne_la) {
    /* no-op for this solver */
    UNUSED_PARAMETER(ne_la);
}
...
create_rtw_linear_algebra_complete(McLinearAlgebraFactory const* factory,
                                   McLinearAlgebra**             linAlg,
                                   PmSparsityPattern const*      pattern,
                                   size_t                        nPerm) {
    ...
    UNUSED_PARAMETER(factory);
    UNUSED_PARAMETER(nPerm);
    ...
    la->mRestart     = &rtw_linalg_restart;
...
PMF_DEPLOY_STATIC McLinearAlgebraStatus
create_rtw_linear_algebra(McLinearAlgebraFactory const* factory,
                          McLinearAlgebra**             linAlg,
                          PmSparsityPattern const*      pattern) {
...
PMF_DEPLOY_STATIC McLinearAlgebraStatus
create_auto_linear_algebra_complete(McLinearAlgebraFactory const* factory,
                                    McLinearAlgebra**             linAlg,
                                    PmSparsityPattern const*      pattern,
                                    size_t                        nPerm) {
    McLinearAlgebraFactory const* sparse_la = mc_get_csparse_linear_algebra();
    McLinearAlgebraFactory const* full_la   = get_rtw_linear_algebra();
...
PMF_DEPLOY_STATIC McLinearAlgebraStatus
create_auto_linear_algebra(McLinearAlgebraFactory const* factory,
                           McLinearAlgebra**             linAlg,
                           PmSparsityPattern const*      pattern) {
