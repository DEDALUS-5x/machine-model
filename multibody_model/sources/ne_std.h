...
;PmRealVector mCR;PmIntVector mCI;PmIntVector mA;PmRealVector mJ;}
...
NE_SYSTEM_INPUT_ID_CI,NE_SYSTEM_INPUT_ID_A,NE_SYSTEM_INPUT_ID_J,
...
neu_get_system_input_sizes(NeSystemInput const*in);NeSystemInput*
...
allocatorPtr);void neu_si_equals_si(NeSystemInput const*destPtr,NeSystemInput
const*srcPtr);NeSystemInput*neu_copy_si(NeSystemInput const*in,PmAllocator*al)
;boolean_T neu_si_equalequals_si(NeSystemInput const*destPtr,NeSystemInput
const*srcPtr);boolean_T neu_sis_equalequals_sis(NeSystemInputSizes const left,
NeSystemInputSizes const right);typedef struct NeDynamicSystemInputTag{
...
;PmIntVector mA;PmRealVector mJ;PmRealVector mW;PmRealVector mS;PmIntVector
...
NE_DYNAMIC_SYSTEM_INPUT_ID_A,NE_DYNAMIC_SYSTEM_INPUT_ID_J,
...
neu_get_dynamic_system_input_sizes(NeDynamicSystemInput const*in);
...
neu_dsi_equals_dsi(NeDynamicSystemInput const*destPtr,NeDynamicSystemInput
const*srcPtr);NeDynamicSystemInput*neu_copy_dsi(NeDynamicSystemInput const*in,
PmAllocator*al);boolean_T neu_dsi_equalequals_dsi(NeDynamicSystemInput const*
destPtr,NeDynamicSystemInput const*srcPtr);boolean_T neu_dsis_equalequals_dsis
(NeDynamicSystemInputSizes const left,NeDynamicSystemInputSizes const right);
...
NE_DIFFOFFSET_EQ_FLAG=1<<7,NE_DISCTIME_EQ_FLAG=1<<8,NE_NO_EQ_FLAGS=0};typedef
struct NeCERDataTag{const char*mFullPath;size_t mIndex;NeCERFlag mFlags;const
char*mObject;size_t mNumRanges;size_t mStart;boolean_T mIsLinear;}NeCERData;
enum{NE_RESERVED_FOR_FUTURE_USE_CER_FLAG=1<<0,NE_NO_CER_FLAGS=0};typedef struct
NeICRDataTag{const char*mFullPath;size_t mIndex;NeICRFlag mFlags;const char*
mObject;size_t mNumRanges;size_t mStart;}NeICRData;enum{NE_INITIAL_ICR_FLAG=1
<<0,NE_NO_ICR_FLAGS=0};typedef struct NeSwitchingCoeffsDataTag{char const*
mFullPath;size_t mIndex;char const*mObject;SscArraySize mSize;char const*
mDescription;}NeSwitchingCoeffsData;typedef struct NeJacobianElemsDataTag{char
const*mFullPath;size_t mIndex;char const*mObject;SscArraySize mSize;char const
*mDescription;}NeJacobianElemsData;typedef struct NeSwitchingStatesDataTag{
size_t mIndex;char const*mJacobFullPath;size_t mJacobIndex;size_t mJacobSize;
size_t mJacobRows;size_t mJacobCols;char const*mCoeffFullPath;size_t
mCoeffIndex;size_t mCoeffSize;size_t mCoeffRows;size_t mCoeffCols;char const*
mDescription;}NeSwitchingStatesData;typedef struct NeVariableDataTag{const char
...
mDescription;}NeVariableData;enum{NE_NODAL_VAR_FLAG=1<<0,NE_OUTPUT_VAR_FLAG=1
<<1,NE_INPUT_VAR_FLAG=1<<2,NE_SIGNAL_VAR_FLAG=1<<3,NE_TIME_VAR_FLAG=1<<4,
NE_INTEG_VAR_FLAG=1<<5,NE_NO_VAR_FLAGS=0};typedef struct NeObservableDataTag{
const char*mFullPath;const char*mObject;SscArraySize mSize;const char*mUnit;
real_T mNominalValue;const char*mNominalUnit;NeNominalSource mNominalSource;
...
const char*mClassName;size_t mBeginLine;size_t mBeginColumn;size_t mEndLine;
...
SscIoInfo const*ioInfoPtr,size_t ioInfoSize){size_t i=0;size_t numSignals=0;
...
extern "C" {
#endif /* __cplusplus */
struct NeSimulationParametersTag{real_T mStartTime;boolean_T
mVariableStepSolver;boolean_T mUsingODEN;boolean_T mFixedStepSolver;real_T
mFixedStepSize;boolean_T mZcDisabled;boolean_T mLoadInitialState;boolean_T
mUseSimState;boolean_T mLinTrimCompile;real_T mRTWModifiedTimeStamp;};
NeSimulationParameters neu_extract_simulation_parameters(NeModelParameters
const*);typedef enum SscLoggingSettingTag{SSC_LOGGING_INVALID= -1,
SSC_LOGGING_OFF,SSC_LOGGING_ON,SSC_LOGGING_LOCAL,SSC_LOGGING_INST,
SSC_LOGGING_NUM}SscLoggingSetting;
#ifdef __cplusplus
}
#endif /* __cplusplus */
#include "pm_std.h"
...
struct NeModelParametersTag{NeSolverType mSolverType;real_T mSolverAbsTol;
real_T mSolverRelTol;boolean_T mVariableStepSolver;boolean_T mIsUsingODEN;
...
NE_MULTIBODY_LOCAL_SOLVER_NUM}NeMultibodyLocalSolverChoice;typedef enum
NeCanonicalOrderingTag{NE_CANONICAL_ORDERING_INVALID= -1,NE_CO_UNORDERED,
NE_CO_CANONICAL,NE_CANONICAL_ORDERING_NUM}NeCanonicalOrdering;typedef enum
NeOptimizeIntermediatesTag{NE_OPTIMIZE_INTERMEDIATES_INVALID= -1,
NE_OPTIMIZE_INTERMEDIATES_UNOPTIMIZED,NE_OPTIMIZE_INTERMEDIATES_OPTIMIZED,
NE_OPTIMIZE_INTERMEDIATES_ENUM}NeOptimizeIntermediates;struct
NeSolverParametersTag{boolean_T mUseExpandedRuntimeScaling;boolean_T
mAllowRtpsInLinearSolver;boolean_T mEnableDiffOffset;boolean_T
mUseMatlabAlgebraicSolver;boolean_T mAllowModesOutsideEqnsInLinearSolver;
boolean_T mAllowSwitchedLinearSolver;boolean_T mStateTrace;boolean_T mProfile;
boolean_T mUseLocalSampling;boolean_T mEnableSwitchedLinearOptims;boolean_T
mFrequencyDomain;boolean_T mUseCCode;real_T mRelTol;real_T mAbsTol;real_T
mMinStep;boolean_T mToWorkspace;boolean_T mRevertToSquareIcSolve;size_t
mNumHomotopyIterations;boolean_T mPhasorMode;size_t mPhasorModeNumHarmonics;
NeConsistencySolver mConsistencySolver;NeCanonicalOrdering mCanonicalOrdering;
NeOptimizeIntermediates mOptimizeIntermediates;NeIndexReductionMethod
mIndexReductionMethod;boolean_T mDoDC;real_T mResidualTolerance;
NeToleranceSource mConsistencyTolSource;real_T mConsistencyAbsTol;real_T
mConsistencyRelTol;real_T mConsistencyTolFactor;boolean_T mUseLocalSolver;
NeLocalSolverChoice mLocalSolverChoice;boolean_T mResolveIndetEquations;size_t
mFunctionEvalNumThread;real_T mLocalSolverSampleTime;boolean_T mDoFixedCost;
size_t mMaxNonlinIter;size_t mMaxModeIter;boolean_T mComputeImpulses;size_t
mImpulseIterations;NeLinearAlgebraChoice mLinearAlgebra;size_t
mLinearAlgebraNumThread;NeEquationFormulationChoice mEquationFormulation;
size_t mDelaysMemoryBudget;boolean_T mAutomaticFiltering;real_T
mFilteringTimeConstant;NePartitionStorageMethod mPartitionStorageMethod;size_t
mPartitionMemoryBudget;NePartitionMethod mPartitionMethod;boolean_T
mMultibodyUseLocalSolver;NeMultibodyLocalSolverChoice
mMultibodyLocalSolverChoice;real_T mMultibodyLocalSolverSampleTime;};
#ifdef __cplusplus
}
#endif /* __cplusplus */
#endif /* __ne_std_h__ */
